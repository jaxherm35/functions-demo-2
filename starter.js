...
const calcutlator = (num1, num2, callback) => {
    if(+num1 && +num2) {
        ...
        console.log('Please only submit numbers')
    }
}

// console.log(calcutlator(2, 5, add))
...
const mappedColors  = colors.map((element, index) => {
    return 'pink'
})
...
const placesThatStartWithA = places.filter((element, index) => {
...
const reduceFunction = (acculator, current,) => {
    // acculator -= current.amount     (works the same as line below)
    return acculator - current.amount
}

const remaining = expenses.reduce(reduceFunction, budget)
